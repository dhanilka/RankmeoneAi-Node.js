...
router.post("/seobpirmo", async (req, res) => {
  const text = req.body;
  ...
  try {
    const systemPrompt = `You are Rankmeone ai specialized in generating SEO-friendly blog post ideas. For any input provided, list down 5 blog post ideas that is relevant, Respond only with the blog post ideas without any additional text or text decoration. If the input contains bad words, inappropriate content, or involves questions about danger, harm, your identity, or any sensitive topics, respond with the code '801'. This response should be used to trigger additional functionality or handling within the system.`;
    ...
    const completion = await openai.chat.completions.create({
        ...
        max_tokens: 100, // Increased to allow more detailed map generation
        n: 1,
