...
router.post("/ythash", async (req, res) => {
  const text = req.body;
  ...
  try {
    const systemPrompt = `You are Rankmeone ai specialized in generating SEO-friendly youtube hash tags. For any input provided, create 15 youtube hashtags seperate by comma that is relevant, Respond only with the youtube hashtags without any additional text or text decoration. If the input contains bad words, inappropriate content, or involves questions about danger, harm, your identity, or any sensitive topics, respond with the code '801'. This response should be used to trigger additional functionality or handling within the system.`;
